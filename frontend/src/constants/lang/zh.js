export default {
    // 네비게이션 바
    title_benefits: "Tripko 会员权益",
    benefit_support: "24/7 全天候支持",
    benefit_guarantee: "最优价格保证",
    benefit_secure: "安全支付",
    benefit_cancel: "免费取消/更改",
    title_popular_dest: "热门目的地",
    unit_from: "起",
    title_need_help: "需要帮助？",
    desc_need_help: "随时联系我们。",
    btn_contact_support: "联系客服",
    // 네비게이션 바
    nav_home: "首页",
    nav_shorts: "短视频",
    nav_game: "街景游戏",
    nav_stays: "住宿",
    nav_places: "地点",
    nav_mytrips: "我的行程",
    // 네비게이션 드롭다운
    nav_mypage: "我的页面",
    nav_logout: "退出登录",
    nav_login: "登录",
    nav_signup: "注册",

    // 로그아웃 확인 모달
    logout_confirm_title: "您确定要退出吗？",
    logout_confirm_desc: "退出后部分功能将无法使用。",
    yes: "是",
    no: "否",
    // 사이드바 전용
    nav_geoquiz: "地理测验上传",
    nav_ai_plan: "AI行程生成",
    nav_column: "本地专栏",
    nav_ticket: "机票预订",
    nav_local_auth: "当地人认证",
    nav_likes: "收藏地点",

    // 리뷰 배너
    banner_review_title: "{dest}之旅怎么样？",
    banner_review_desc: "分享您的美好回忆！",
    banner_review_btn: "写点评",
    nav_settings: "设置",
    nav_support: "客户中心",
    nav_version: "版本信息",

    // 메인 페이지 히어로
    hero_title_1: "下个目的地？",
    hero_title_2: "韩国等你来。",
    search_placeholder: "搜索目的地、机票、酒店...",
    search_btn: "搜索",

    // 히어로 탭 & 일정
    hero_tab_search: "搜索",
    hero_tab_plan: "行程制作",
    plan_title_ph: "旅行标题 (例: 暑假)",
    plan_start: "出发日期",
    plan_end: "返回日期",
    plan_create_btn: "开始规划",

    // 필터 칩
    filter_all: "全部",
    filter_places: "地点",
    filter_geoquiz: "地理测验",
    filter_stays: "住宿",
    filter_flights: "交通",

    // 빠른 작업
    action_flight: "预订机票",
    title_transport: "交通",
    action_stays: "查找住宿",
    action_places: "查找景点",
    action_geoquiz: "地理测验",

    // 섹션
    sec_upcoming: "即将开始的旅程",
    btn_view_all: "查看全部",
    sec_trending: "热门短视频 & 城市",

    // 쇼츠 페이지 & 상세
    shorts_title: "短视频",
    shorts_sub: "通过AI字幕/翻译观看最新上传。",
    btn_upload: "上传",
    btn_batch: "批量优化",
    loading: "加载中...",
    no_shorts: "没有视频。请登录并上传。",
    duration_missing: "00:00",
    now_playing: "正在播放",
    lang_label: "语言",

    // 공통
    play: "播放",
    close: "关闭",
    follow: "关注",
    share: "分享",
    save: "保存",
    comments: "评论",
    msg_no_comments: '暂无评论。',
    add_comment: "添加评论...",

    // 인증 페이지
    login_title: "登录",
    login_subtitle: "开启您的个性化旅行体验。",
    signup_title: "注册",
    signup_subtitle: "立即加入，开始您的旅程。",
    email_label: "电子邮件",
    password_label: "密码",
    password_confirm_label: "确认密码",
    nickname_label: "昵称",
    birth_year_label: "出生年份",
    country_label: "国家",
    country_placeholder: "例如: 韩国",
    city_label: "城市",
    login_btn: "登录",
    signup_btn: "注册",
    google_login: "通过Google继续",
    kakao_login: "通过Kakao继续",
    no_account: "还没有账号？",
    have_account: "已有账号？",
    forgot_password: "忘记密码？",
    registration_success: "注册成功！请登录。",

    // 마이페이지
    mypage_title: "我的页面",
    mypage_desc: "管理您的账号和活动。",
    tab_profile: "个人资料",
    tab_shorts: "我的短视频",
    tab_schedules: "行程",
    tab_columns: "专栏",
    tab_reservations: "预订",
    tab_preferences: "设置",
    tab_saved: "收藏",
    tab_reviews: "评论",

    // 当地人徽章
    local_badge_title: "当地人徽章",
    badge_since: "认证开始",
    badge_months: "持续月数",
    badge_months_unit: "个月",
    badge_level: "等级",
    badge_writer: "撰稿人",
    badge_explorer: "探险家",
    no_local_badge: "还没有当地人徽章。",
    no_local_badge_desc: "在当地居住一段时间即可获得徽章！",

    // 我的页面追加翻译
    profile_update_failed: "更新个人资料失败。",
    confirm_remove_bookmark: "确定要从书签中删除此地点吗？",
    remove_bookmark_failed: "删除书签失败",
    confirm_delete_review: "确定要删除此评论吗？",
    delete_review_failed: "删除评论失败",
    upload_first_short: "上传您的第一个短视频！",
    create_first_plan: "创建您的第一个旅行计划！",
    unit_days: "天",
    view_details: "查看详情 →",
    read_more: "阅读更多 →",
    preferences_desc: "语言和货币设置。",
    label_language: "语言",
    label_currency: "货币",
    status_pending: "待处理",
    status_cancelled: "已取消",
    no_columns_yet: "还没有专栏",
    columns_empty_desc: "与旅行者分享当地信息！",
    write_column: "写专栏",
    my_columns: "我的专栏",
    no_saved_places: "没有保存的地点",
    saved_places_empty_desc: "收藏您稍后想访问的地点！",
    explore_places: "探索地点",
    saved_places: "保存的地点",
    btn_remove: "删除",
    no_reviews_yet: "还没有评论",
    reviews_empty_desc: "与他人分享您的体验！",
    find_places_to_review: "查找要评论的地点",
    my_reviews: "我的评论",
    default_nickname: "旅行者",
    na: "N/A",

    profile_basic: "基本信息",
    profile_basic_desc: "向其他用户显示的个人资料信息。",
    edit_btn: "编辑",
    save_btn: "保存",
    cancel_btn: "取消",

    id_label: "ID",
    location_label: "位置",
    phone_label: "电话号码",

    // 쇼츠 업로드
    upload_title_new: "上传短视频",
    upload_title_edit: "编辑短视频",
    upload_sub_new: "与世界分享您的瞬间",
    upload_sub_edit: "更新已有视频信息",
    back: "返回",
    req_login: "需要登录",
    load_fail: "加载视频信息失败。",
    upload_success: "上传成功！",
    update_success: "更新成功！",
    submit_fail: "提交失败。",
    label_video: "视频文件",
    label_optional: "（可选）",
    label_required: "（必填）",
    label_title: "标题",
    label_desc: "描述",
    ph_title: "输入吸引人的标题...",
    ph_desc: "关于这个视频的描述？",
    video_drag: "点击或拖拽视频至此",
    video_format: "MP4, WebM, Ogg (最大 50MB)",
    video_change: "更改视频",
    video_selected: "已选择",
    btn_uploading: "上传中...",
    btn_updating: "更新中...",
    btn_save_changes: "保存更改",
    btn_upload_short: "上传视频",
    // 장소 검색
    place_search_title: "地点搜索",
    place_search_placeholder: "你想去哪里？",
    shorts_upload_desc_placeholder: "是关于什么的视频？",
    shorts_location_placeholder: "例如: #首尔 #韩国",
    search_results_title: "'{query}' 搜索结果",
    search_result_count: "找到 {count} 个地点。",
    search_initial_msg: "搜索你想去的目的地。",
    no_results_title: "未找到结果",
    no_results_desc_try: "尝试使用不同的关键词搜索。",
    searching: "搜索中...",

    // 장소 상세
    place_basic_info: "基本信息",
    place_opening_hours: "营业时间",
    place_location: "位置",
    place_rating: "评分",
    place_reviews: "{count} 条评论",
    btn_add_to_plan: "添加到行程",
    btn_write_review: "写评论",
    msg_login_required: "需要登录。前往登录页面？",
    msg_feature_coming_soon: "{action} 即将推出。",
    msg_bookmark_error: "书签更新失败。请重试。",
    msg_no_image: "暂无图片",
    loading_place: "正在加载地点详情...",
    btn_go_back: "返回",

    // 계획 목록
    plan_list_title: "我的旅行行程",
    plan_list_subtitle: "规划和管理您的独特旅程",
    btn_ai_recommend: "获取AI推荐",
    btn_create_new: "创建新行程",
    msg_no_plans: "暂无旅行行程",
    btn_create_first: "创建您的第一个行程",

    // 계획 생성 페이지
    plan_create_title: "创建新旅行计划",
    plan_create_subtitle: "规划您自己的特别旅行",
    label_trip_title: "旅行标题",
    placeholder_trip_title: "例如：济州岛治愈之旅",
    label_trip_desc: "旅行说明",
    placeholder_trip_desc: "请输入旅行的简要说明",
    label_start_date: "开始日期",
    label_end_date: "结束日期",
    label_public: "设为公开旅行",
    desc_public: "其他用户可以查看此旅行计划",

    btn_submit_create: "创建旅行计划",
    btn_creating: "创建中...",
    info_after_create_title: "创建计划后",
    info_after_create_1: "您可以添加旅行地点",
    info_after_create_2: "您可以按日期组织日程",
    info_after_create_3: "您可以为每个地点添加备注和图片",
    alert_login_required_service: "此服务需要登录。",
    alert_input_title: "请输入旅行标题。",
    alert_select_dates: "请选择旅行日期。",
    alert_date_order: "结束日期必须在开始日期之后。",
    msg_create_success: "旅行计划已创建！",
    msg_create_fail: "创建旅行计划失败。",

    // 장소 추가/수정 / 계획 수정 / AI 추천
    // 장소 관련
    title_add_place: "添加地点",
    subtitle_add_place: "向{planTitle}添加新地点",
    title_edit_place: "编辑地点",
    subtitle_edit_place: "编辑{placeName}的信息",
    label_place_name: "地点名称",
    placeholder_place_name: "例如：景福宫、南山塔",
    label_visit_date: "访问日期",
    label_visit_order: "访问顺序",
    desc_visit_order: "同一日期内的访问顺序（从0开始）",
    label_memo: "备注",
    placeholder_memo: "请输入此地点的备注（选填）",
    msg_autocomplete_guide: "输入地点名称以显示自动完成列表",
    msg_autocomplete_guide_2: "输入地点名称以显示自动完成搜索结果",
    msg_searching: "搜索中...",
    msg_no_results: "无搜索结果",
    label_current_address: "当前地址（参考）",
    desc_address_update: "更改地点名称时地址也会更新",
    btn_add_place: "添加地点",
    btn_adding: "添加中...",
    btn_save: "保存",
    btn_saving: "保存中...",
    btn_back: "返回",
    btn_back_to_list: "返回列表",
    msg_selected: "已选择",
    alert_place_name_required: "请输入地点名称。",
    alert_visit_date_required: "请选择访问日期。",
    alert_date_range_error: "日期必须在{startDate}和{endDate}之间。",
    alert_add_place_fail: "添加地点失败。",
    msg_loading: "加载中...",
    msg_plan_not_found: "未找到计划。",

    // 계획 수정
    title_plan_edit: "编辑旅行计划",
    subtitle_plan_edit: "编辑旅行计划的基本信息",
    label_plan_type: "计划类型",
    placeholder_plan_title: "请输入旅行计划标题",
    placeholder_plan_desc: "请输入计划说明（选填）",
    title_date_warning: "更改日期时的注意事项",
    desc_date_warning: "此计划已包含{count}个地点。更改日期可能会导致现有地点的日期超出计划范围。",

    // AI 추천
    title_ai_recommend: "AI旅行推荐",
    subtitle_ai_recommend: "AI将为您创建完美的旅行日程",
    q_destination: "您要去哪里旅行？",
    q_date: "您何时去旅行？",
    q_style: "您想要什么样的旅行？",
    q_additional: "还有其他要求吗？",
    placeholder_additional: "例如：可以带宠物的地方、适合家庭游客的地方、拍照胜地等",
    msg_total_days: "共{days}天旅行",
    btn_get_ai_recommend: "获取AI推荐",
    btn_ai_generating: "AI正在生成您的计划...",
    info_ai_how_title: "AI推荐如何工作？",
    info_ai_how_1: "AI根据您输入的信息创建最佳旅行日程",
    info_ai_how_2: "按日期自动添加访问地点和说明",
    info_ai_how_3: "您可以随时修改生成的日程",
    info_ai_how_4: "AI生成大约需要30秒到1分钟",
    alert_dest_required: "请选择目的地。",
    alert_style_required: "请选择旅行风格。",
    alert_ai_request_fail: "请求AI推荐失败。",
    alert_ai_polling_fail: "AI状态确认失败。",

    // 여행지
    dest_gapyeong_yangpyeong: "加平/杨平",
    dest_gangneung_sokcho: "江陵/束草",
    dest_gyeongju: "庆州",
    dest_busan: "釜山",
    dest_yeosu: "丽水",
    dest_incheon: "仁川",
    dest_jeonju: "全州",
    dest_jeju: "济州",
    dest_chuncheon_hongcheon: "春川/洪川",
    dest_taean: "泰安",

    // 여행 스타일
    style_healing: "治愈/休养",
    style_healing_desc: "安静舒适的旅行",
    style_activity: "活动",
    style_activity_desc: "充满活力和动态的旅行",
    style_culture: "文化/历史",
    style_culture_desc: "感受历史和文化的旅行",
    style_food: "美食之旅",
    style_food_desc: "以探访美食为主的旅行",
    style_nature: "自然景观",
    style_nature_desc: "尽情享受大自然的旅行",
    badge_ai: "AI推荐",
    badge_manual: "手动",
    badge_public: "公开",
    badge_private: "私密",
    no_desc: "无描述。",
    trip_days: "{days} 天行程",
    btn_edit: "编辑",
    msg_confirm_delete: "确定要删除此行程吗？",
    msg_delete_fail: "删除失败。",

    // 공통 알림
    alert_login_required: "访问此服务需要登录。",
    alert_public_like_only: "只有公开的行程才能点赞。",
    alert_like_fail: "点赞更新失败。",
    alert_public_comment_only: "只有公开的行程才能评论。",
    alert_comment_fail: "评论发表失败。",
    alert_comment_update_fail: "评论更新失败。",
    confirm_comment_delete: "确定要删除此评论吗？",
    alert_comment_delete_fail: "删除评论失败。",
    msg_load_fail: "加载旅行计划失败。",

    // 계획 상세
    error_invalid_access: "无效访问。",
    error_plan_private: "此行程为私密行程。",
    error_plan_not_found: "未找到行程。",
    error_fetch_plan_failed: "获取行程失败。",
    msg_confirm_delete_place: "确定要删除此地点吗？",
    error_delete_place_failed: "删除地点失败。",

    btn_edit_plan: "编辑行程",

    msg_no_places_added: "尚未添加地点。",
    msg_no_places_date: "该日期尚未添加地点。",
    default_place_name: "无地点名称",
    alt_place_image: "地点图片 {index}",
    day_label: "第 {day} 天",

    // 메인 페이지 신규
    cta_start_journey: "开始您的下一段旅程！",
    cta_no_upcoming: "您没有即将开始的行程。为何不现在规划一个？",
    btn_create_trip: "创建行程",
    label_personal_trip: "个人行程",
    label_group_trip: "团体行程",
    link_view_itinerary: "查看行程 →",
    category_travel: "旅行",
    // 숙소 지도
    stays_title: "查找住宿",
    stays_search_placeholder: "搜索位置 (例如: 江南)",
    stays_search_btn: "搜索",
    stays_type_label: "类型:",
    stays_type_all: "所有住宿",
    stays_type_hotel: "酒店",
    stays_type_motel: "汽车旅馆",
    stays_type_pension: "养老金(民宿)",
    stays_type_guesthouse: "宾馆",
    stays_type_resort: "度假村",
    stays_loading_map: "加载地图中...",
    stays_error_map: "地图加载错误",
    stays_initial_msg: "点击地图搜索住宿。",
    stays_searching: "搜索中...",
    stays_found_count: "找到 {count} 个住宿",
    stays_no_results: "附近未找到住宿",
    stays_updating: "更新结果中...",
    stays_finding: "正在查找地点...",
    stays_view_details: "查看详情 ↗",

    // 지오 이미지 업로더
    geo_title: "AI位置分析",
    geo_desc: "上传旅行照片以提取GPS位置并开始街景游戏。",
    geo_drag_drop: "点击或拖放上传",
    geo_support_fmt: "支持 JPG, PNG, HEIC (含位置数据)",
    geo_remove: "移除并重新上传",
    geo_analyzing: "正在分析照片元数据...",
    geo_lat: "纬度",
    geo_lng: "经度",
    geo_success: "位置数据提取成功！",
    geo_start_game: "开始街景游戏",
    geo_or_memories: "或从回忆中选择",
    geo_my_reviews: "我的照片评论",
    geo_no_reviews: "暂无照片评论。",
    geo_write_review_hint: "带照片的评论将显示在这里！",
    geo_start_with: "从“{name}”开始",
    geo_back_home: "← 返回主页",

    // 로컬 칼럼 목록
    col_title_main: "本地专栏",
    col_desc_1: "发现真正的当地人推荐的隐藏景点和美食故事。",
    col_desc_2: "充满只有经过验证的当地人才能提供的特别信息。",
    col_write_btn: "写专栏",
    col_fetch_error: "加载专栏失败。",
    col_retry: "重试",
    col_no_cols: "暂无专栏。",
    col_be_first: "成为第一个写专栏的人！",
    col_anonymous: "匿名",
    // 쇼츠
    shorts_load_error: "加载短视频失败。",

    // 항공권 검색
    flight_title: "交通",
    flight_search_title: "机票搜索",
    flight_oneway: "单程",
    flight_roundtrip: "往返",
    flight_dep: "出发地",
    flight_arr: "目的地",
    flight_dep_date: "出发日期",
    flight_ret_date: "返回日期",
    flight_airline: "航空公司 (可选)",
    flight_airline_all: "所有航空公司",
    flight_passengers: "选择人数",
    flight_adult: "成人",
    flight_adult_desc: "12岁及以上",
    flight_child: "儿童",
    flight_child_desc: "2岁~11岁",
    flight_infant: "婴儿",
    flight_infant_desc: "2岁以下, 需成人陪同",
    flight_search_btn: "搜索航班",
    flight_searching: "搜索中...",
    flight_tip_title: "搜索提示",
    flight_tip_1: "提前7天预订可享最高30%优惠",
    flight_tip_2: "平日航班比周末更便宜",
    flight_tip_3: "检查清晨/深夜时段的特价航班",
    flight_alert_fill_all: "请选择出发地、目的地和日期。",
    flight_alert_same_airport: "出发地和目的地不能相同。",
    flight_alert_past_date: "不能选择过去的日期。",
    flight_alert_ret_req: "您选择了往返，请选择返回日期。",
    flight_alert_ret_after: "返回日期必须在出发日期之后。",
    flight_placeholder_dep: "\u9009\u62E9\u51FA\u53D1\u673A\u573A",
    flight_placeholder_arr: "\u9009\u62E9\u5230\u8FBE\u673A\u573A",
    tab_flight: "航班",
    tab_train: "火车",
    tab_subway: "地铁",
    tab_bus: "巴士",
    tab_taxi: "出租车",
    flight_load_error: "加载数据失败。",

    // 거리뷰 게임
    game_title: "街景",
    game_target_photo: "目标照片",
    game_round: "轮次",
    game_time: "时间",
    game_question: "这是哪里？",
    game_instruction: "在地图上找出照片中的位置！",
    game_pin_placed: "已放置标记！",
    game_btn_confirm: "确认猜测",
    game_btn_next: "下一轮",
    game_result_title: "本轮结果",
    game_distance: "距离",
    game_score: "得分",
    game_over_title: "游戏结束！",
    game_total_score: "您的总分:",
    game_view_place: "查看地点 →",
    game_btn_home: "回首页",
    game_btn_play_again: "再玩一次",
    game_loading_new: "正在寻找新地点...",
    game_error_info: "无法加载地点信息。",
    game_warn_distance: "附近没有街景，显示约{d}米外的视图。",
    game_no_pano_title: "未找到街景",
    game_no_pano_desc: "附近没有道路数据，或者是该地点在室内。",

    // 로컬 칼럼
    col_write_title: "撰写新专栏",
    col_edit_title: "编辑专栏",
    col_input_title: "标题",
    col_input_title_ph: "请取一个有吸引力的标题",
    col_input_thumb: "缩略图 (代表图片)",
    col_input_intro: "序言 (Intro)",
    col_input_intro_ph: "请撰写专栏的开篇。",
    col_input_intro_img: "序言图片 (可选)",
    btn_add_img: "添加图片",
    btn_delete: "删除",
    col_section_n: "章节",
    btn_del_section: "删除章节",
    col_input_subtitle_ph: "请输入小标题",
    col_input_content_ph: "请撰写内容",
    col_related_place: "相关地点",
    col_place_ph: "搜索地点名称 (例如:南山塔)",
    btn_add_section: "添加章节",
    btn_cancel: "取消",
    btn_submit_create_column: "撰写完成",
    btn_submit_edit: "修改完成",
    err_title_req: "请输入标题。",
    err_thumb_req: "缩略图是必需的。",
    err_section_min: "至少需要一个章节。",
    msg_col_created: "专栏已创建。",
    msg_col_updated: "专栏已修改。",
    msg_save_fail: "保存失败。",

    // 인증 모달
    auth_modal_title: "当地人认证",
    auth_modal_desc: "根据当前位置获取当地人徽章！",
    btn_auth_my_loc: "使用我的位置认证",
    msg_loc_fetching: "正在获取位置信息...",
    msg_loc_not_support: "浏览器不支持位置信息。",
    msg_auth_checking: "正在与认证服务器通信...",
    msg_auth_success: "认证成功！",
    msg_auth_fail: "认证过程中发生错误。",
    msg_loc_perm_denied: "请允许提供位置信息。",
    msg_loc_unavailable: "无法使用位置信息。",
    msg_loc_timeout: "位置信息请求超时。",
    msg_loc_fail: "获取位置信息失败。",

    // 상세 & 목록
    col_view_place: "查看地点信息",
    btn_back_list: "返回列表",
    confirm_delete_col: "确定要删除此专栏吗？",
    msg_col_deleted: "专栏已删除。",
    msg_del_fail: "删除失败，请重试。",
    col_views: "浏览",
    msg_auth_inactive: "当地人徽章未激活。",
    msg_auth_level_low: "当地人等级3以上可撰写。",
    msg_auth_error: "无法确认权限信息。",
    col_upload_thumb: "点击上传图片",
    col_intro_img_add: "添加图片",

    // 도시 페이지
    city_search_title: "你要去哪里？ ✈️",
    city_search_desc: "搜索城市名称，查看综合旅游信息。",
    city_search_ph: "搜索城市 (例如: 首尔, 釜山)",
    city_popular_title: "热门目的地 🔥",
    city_header_desc: "我们收集了这个城市的所有旅游信息。",
    city_sec_plan: "{city} 旅行行程",
    city_sec_place: "{city} 推荐地点",
    city_empty_plan: "暂时没有{city}的旅行行程。",
    city_empty_place: "暂时没有{city}的注册地点。",
    city_empty_shorts: "没有相关的短视频。",
    city_empty_column: "暂时没有{city}的本地专栏。",
    city_error_title: "发生错误",
    city_btn_retry_search: "搜索其他城市",
    city_load_error: "加载内容失败。",
    flight_provider_check_required: "需要确认供应商",

    // 통화
    unit_krw: "韩元",
    date_format_ymd: "{year}年{month}月{day}日",
    unit_people: "人",

    // 날짜/시간 형식
    datetime_format_ymdhm: "{year}年{month}月{day}日 {hours}:{minutes}",

    // 결제 확인 중
    payment_confirming_title: "正在处理付款",
    payment_confirming_desc: "正在安全处理您的付款。",
    payment_confirming_wait: "请稍候。",

    // 결제 성공
    payment_success_title: "付款完成",
    payment_success_desc: "您的预订已成功处理。",
    payment_order_number: "订单号",
    payment_amount_label: "付款金额",
    payment_redirecting: "即将跳转到确认页面...",

    // 결제 실패
    payment_error_title: "付款失败",
    payment_error_invalid_params: "付款信息无效。",
    payment_error_refund_info: "如果付款批准失败但已扣款，将自动取消。退款将在3-5个工作日内处理。",

    // 버튼
    btn_restart: "重新开始",
    btn_processing: "处理中...",

    // 예약 상세 모달
    reservation_detail_title: "预订详情",
    reservation_loading: "加载中...",
    reservation_load_error: "无法加载信息。",
    reservation_detail_error: "加载预订详情失败。",
    label_order_number: "订单号",
    label_status: "状态",
    status_confirmed: "已确认",
    label_payment_amount: "支付金额",
    label_departure_time: "出发时间",
    label_reservation_time: "预订时间",
    title_flight_detail: "航班信息",
    label_trip_type: "行程类型",
    trip_oneway: "单程",
    trip_roundtrip: "往返",
    label_cabin_class: "座位等级",
    label_passengers_count: "乘客人数",
    passenger_adult: "成人",
    passenger_child: "儿童",
    passenger_infant: "婴儿",
    title_flight_segments: "航班航段",
    unit_minutes: "分钟",
    title_passenger_info: "乘客信息",
    title_seat_info: "座位信息",
    seat_unassigned: "未分配",
    btn_close: "关闭",
    label_order: "订单",

    // 일정 추가 모달
    title_add_schedule: "添加到日程",
    btn_new_plan: "+ 新建行程",
    msg_add_failed: "添加日程失败。",
    label_current_trip: "当前行程",
    label_upcoming_trips: "即将到来的行程",
    msg_no_upcoming_plans: "没有即将到来的行程。",
    msg_adding: "添加中...",
    btn_add_to_selected_schedule: "添加到日程",

    // 장소 상세 페이지
    error_load_place: "无法加载地点信息。",


    // 로컬 칼럼 목록
    col_title: "本地专栏",
    place_search_placeholder_column: "输入搜索内容...",
    no_results_desc: "没有找到结果：",
    btn_view_all_columns: "查看全部",
    label_place: "地点",

    // 리뷰 섹션
    label_reviews: "评论",
    msg_login_required_review: "请登录后撰写评论。",
    msg_editing_review: "正在编辑评论",
    label_rating: "评分",
    placeholder_review_content: "请留下您对这个地方的真实评论。",
    btn_add_photo: "添加照片",
    msg_saving: "保存中...",
    btn_update_complete: "修改完成",
    btn_submit_review: "提交评论",
    sort_latest: "最新",
    sort_rating_desc: "评分最高",
    sort_rating_asc: "评分最低",
    msg_no_reviews: "还没有评论。成为第一个留下评论的人吧！",
    label_anonymous_user: "匿名用户",
    btn_load_more: "加载更多",
    msg_review_updated: "评论已更新。",
    msg_review_created: "评论已提交。",
    msg_review_save_failed: "保存评论失败。",
    msg_confirm_delete_review: "确定要删除此评论吗？",
    msg_review_deleted: "评论已删除。",
    msg_review_delete_failed: "删除评论失败。",

    // 기차 종류
    train_type_ktx: "KTX",
    train_type_srt: "SRT",
    train_type_itx_saemaeul: "ITX-新村号",
    train_type_itx_cheongchun: "ITX-青春号",
    train_type_saemaeul: "新村号",
    train_type_mugunghwa: "无穷花号",

    // 기차 검색
    title_transport: "交通",
    title_train_search: "火车搜索",
    label_dep_station: "出发站",
    label_arr_station: "到达站",
    label_train_type: "火车类型（可选）",
    placeholder_select_dep_station: "选择出发站",
    placeholder_select_arr_station: "选择到达站",
    desc_train_type_all: "不选择则搜索所有火车类型",
    btn_search_train: "搜索火车",
    btn_searching_train: "搜索中...",
    alert_fill_all_train_search: "请选择出发站、到达站和日期。",
    alert_same_station: "出发站和到达站不能相同。",
    alert_past_date: "不能选择过去的日期。",
    title_train_guide: "火车预订指南",
    info_train_1: "本服务仅提供约1周内的火车信息",
    info_train_2: "请在KORAIL/SRT网站查看准确的火车时刻和座位信息",
    info_train_3: "可在KORAIL或SRT网站进行预订",
    info_train_4: "KTX、ITX、新村号可在KORAIL预订",
    info_train_5: "SRT仅可在SRT网站预订",

    // 기차 검색 결과
    msg_searching_trains: "正在搜索火车...",
    title_search_results_count: "搜索结果（{count}条）",
    msg_no_trains_found: "未找到火车。请尝试其他条件。",
    msg_train_search_error: "搜索火车时发生错误。",
    btn_change_conditions: "更改搜索条件",
    label_adult_fare: "成人票价",
    btn_book_ticket: "预订",
    msg_no_trains_empty: "未找到火车",
    btn_search_other_conditions: "使用其他条件搜索",
    title_booking_guide: "预订指南",
    info_booking_1: "点击预订按钮将跳转到外部预订网站",
    info_booking_2: "KTX、ITX、新村号、无穷花号请在KORAIL网站预订",
    info_booking_3: "SRT仅可在SRT网站预订",
    info_booking_4: "在预订网站选择火车和座位完成预订",
    info_booking_5: "支付在外部网站进行",
    train_type_all: "全部",
    region_capital: "首都圈",
    region_chungcheong: "忠清",
    region_gyeongsang: "庆尚",
    region_jeolla: "全罗",
    region_gangwon: "江原",

    // 지하철
    title_subway_search: "地铁路线搜索",
    label_search_option: "搜索选项",
    placeholder_input_dep_station: "输入出发站",
    placeholder_input_arr_station: "输入到达站",
    option_fast: "最快",
    option_few_transfer: "最少换乘",
    option_cheap: "最低票价",
    btn_search_route: "搜索路线",
    alert_fill_all_subway_search: "请输入出发站和到达站。",
    title_subway_guide: "地铁搜索指南",
    info_subway_1: "搜索首尔、釜山、大邱、光州、大田的地铁路线。",
    info_subway_2: "提供最多3条最优路线。",
    info_subway_3: "查看所需时间、换乘次数和票价。",
    info_subway_4: "在Kakao地图上查看详细路线。",
    info_subway_5: "查看车站显示屏获取实时到达信息。",

    // 지하철 경로
    msg_searching_route: "正在搜索最优路线...",
    title_recommend_routes: "推荐路线（{count}条）",
    label_route_n: "路线{n}",
    badge_recommend: "推荐",
    label_duration: "所需时间",
    label_transfers: "换乘",
    label_fare: "票价",
    unit_hour: "小时",
    unit_min: "分钟",
    unit_times: "次",
    desc_route_step: "{stations}站 · 约{duration}分钟",
    msg_no_route_found: "未找到路线。",
    msg_no_route_found_detail: "未找到路线。请检查站名。",
    msg_subway_search_error: "搜索路线时发生错误。",
    btn_search_again: "重新搜索",
    title_usage_guide: "使用指南",
    info_route_1: "路线可能根据实时情况发生变化。",
    info_route_2: "票价基于交通卡。",

    // Search Page
    see_all: "查看全部",
    search_tab_all: "全部",
    search_tab_places: "地点",
    search_tab_plans: "行程",
    search_tab_shorts: "短视频",
    search_tab_reviews: "评论",
    search_tab_others: "其他",
    search_header_places: "推荐地点",
    search_header_shorts: "短视频",
    search_header_plans: "热门行程",
    search_header_flights: "机票",
    search_header_transports: "交通",
    search_header_reviews: "旅客评论",
    search_header_others: "其他结果",
    search_loading: "正在搜索 \"{query}\"...",
    search_no_results_title: "准备好了吗？",
    search_no_results_desc: "搜索目的地、酒店或向导。",
    search_empty_title: "没有找到关于 \"{tab}\" 的结果。",
    search_empty_desc: "尝试调整您的搜索或筛选条件。",
    info_route_3: "请在各站查看首末班车时间。",
    info_route_4: "查看车站显示屏获取实时到达信息。",
    label_provider: "提供者：",

    // 页脚
    footer_desc: "通过AI行程和本地信息发现韩国的最佳体验。",
    footer_explore: "探索",
    footer_destinations: "目的地",
    footer_trending_shorts: "热门短视频",
    footer_hotels: "酒店",
    footer_company: "公司",
    footer_about: "关于我们",
    footer_contact: "联系我们",
    footer_privacy: "隐私政策",
    footer_stay_updated: "获取更新",
    footer_copyright: "Tripko Inc. All rights reserved.",

    // 认证 - 登录
    remember_me: "记住我",
    sns_login: "社交账号登录",
    login_failed: "登录失败。请检查您的凭据。",

    // 认证 - 注册
    passwords_not_match: "密码不匹配",
    password_min_length: "密码至少需要8个字符",
    registration_failed: "注册失败。请重试。",
    network_error: "发生网络错误。",
    email_placeholder: "name@example.com",
    phone_placeholder: "+82 10-1234-5678",
    year_placeholder: "YYYY",

    // 侧边栏
    sidebar_title: "韩国之旅",
    sidebar_copyright: "Tripko 平台",

    // 计划详情 - 图片
    alert_image_only: "只能上传图片文件。",
    alert_image_size: "图片大小不能超过10MB。",
    alert_image_upload_fail: "图片上传失败。",
    confirm_image_delete: "确定删除此图片吗？",
    alert_image_delete_fail: "图片删除失败。",
    btn_add_image: "添加照片",
    anonymous: "匿名"
};
