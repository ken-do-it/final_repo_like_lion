...
def plan_list_create(request):
    ...
    GET /api/plans/
    - 모든 여행 일정 목록 조회
    - 쿼리 파라미터: is_public=true/false, user=<user_id>, lang=<lang_code>
    """
    if request.method == 'GET':
        ...
        if target_lang:
            try:
                ...
                logger.error(f"Plan translation failed: {e}")
...
def plan_retrieve_update_delete(request, plan_id):
    """
    여행 일정 조회 / 수정 / 삭제
    GET /api/plans/<plan_id>/?lang=<lang>
    """
    try:
...
def detail_list_create(request, plan_id):
    """
    날짜별 장소 목록 조회 / 추가
    GET /api/plans/<plan_id>/details/?lang=<lang>
    """
    try:
        plan = TravelPlan.objects.get(id=plan_id)
    except TravelPlan.DoesNotExist:
    ...
    if request.method == 'GET':
        ...
        if target_lang:
            try:
                ...
                logger.error(f"PlanDetails list translation failed: {e}")
...
def detail_retrieve_update_delete(request, detail_id):
    """
    장소 조회 / 수정 / 삭제
    GET /api/plans/details/<detail_id>/?lang=<lang>
    """
    try:
    ...
    if request.method == 'GET':
        ...
        if target_lang:
            try:
                ...
                logger.error(f"PlanDetail retrieve translation failed: {e}")
        ...
        if target_lang:
            try:
                data = TranslationService.apply_translation_batch(
                    data,
                    target_lang,
                    entity_type="plan_comment",
                    fields={'content': 'content_translated'}
                )
            except Exception as e:
                logger.error(f"Plan comment translation failed: {e}")
